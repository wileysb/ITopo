...
def Horizon_shade(project_parameters, lat, lon, yday, utc_hour):
    ...
    az_grid_int = np.round(s_az_fl, 0)

    az_list = np.unique(az_grid_int)
    # zen = np.unique(sza)

    for azimuth in az_list:
        horizon = gdal_load(project_parameters['hor'].format(float(azimuth)))
        azi_mask = (az_grid_int == azimuth)

        shade_map[azi_mask] = ( (90-sza_fl[azi_mask]) > horizon[azi_mask])
